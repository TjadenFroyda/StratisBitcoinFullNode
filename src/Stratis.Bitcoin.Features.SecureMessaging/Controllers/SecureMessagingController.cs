...
namespace Stratis.Bitcoin.Features.SecureMessaging.Controllers
{
    ...
    public partial class SecureMessagingController : Controller
    {
        ...
        /// <returns></returns>
        internal Wallet.Wallet RecoverWalletFromSeed(ExtKey sharedSecret, string password, string name, DateTime creationTime)
        {
            return this.fullNode.NodeService<WalletManager>().RecoverWallet(sharedSecret, password, name, creationTime);
        }
        ...
        internal Key GetPrivateMessagingKey(SecureMessageKeyRequest request)
        {
            ...
            {
                //This is a base58 encoded string generated by towif(). 
                string encryptedSeed = this.walletManager.GetWalletByName(request.WalletName).EncryptedSeed;

                //This is basically the dumpkey RPC call.  
        ...
        internal IActionResult BuildAndSendTransactionBatch(SecureMessageRequest request)
        {
            ...
            // TODO: Add final check to see if balance is sufficient. 
